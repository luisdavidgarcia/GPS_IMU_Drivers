...
void IMU::begin() const {
	...
	uint8_t set_4g = 0x08;
	status = HAL_I2C_Mem_Write(m_hi2c_, m_mpu9250_address_ << BYTE, ACCEL_CONFIG,
...
void IMU::readAccelerometer() {
	...
	HAL_StatusTypeDef status = HAL_I2C_Mem_Read(m_hi2c_, m_mpu9250_address_ << BYTE, ACCEL_XOUT_H,
...
void IMU::readGyroscope() {
	...
	HAL_StatusTypeDef status = HAL_I2C_Mem_Read(m_hi2c_, m_mpu9250_address_ << BYTE, GYRO_XOUT_H,
